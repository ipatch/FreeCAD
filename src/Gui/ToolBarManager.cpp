...
void ToolBarManager::setState(const QList<QString>& names, State state)
{
    ...
    QString stateStr;
    switch (state) {
        case State::SaveState:        stateStr = "SaveState"; break;
        case State::RestoreDefault:   stateStr = "RestoreDefault"; break;
        case State::ForceAvailable:   stateStr = "ForceAvailable"; break;
        case State::ForceHidden:      stateStr = "ForceHidden"; break;
        default:                       stateStr = "Unknown"; break;
    }
    Base::Console().message("ToolBarManager::setState [%s] for %d toolbars: %s\n",
...
#ifdef FC_DEBUG
        Base::Console().message("  visibility(%s, default=%d) = %d\n",
                            name.toUtf8().constData(), defaultvalue, result);
#endif
        return result;
    };
