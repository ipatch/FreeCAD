...
TopoShape::sortEdges(std::list<TopoShape>& edges, bool keepOrder, double tol)
{
    ...
    auto reverseEdge = [](const TopoShape &edge) {
        ...
        const char *edgeName = "Edge1";
        auto mapped = edge.getElementName(edgeName, MapToNamed);
        if (mapped != edgeName)
            res.setElementName(edgeName, mapped);
        const char *v1Name = "Vertex1";
        const char *v2Name = "Vertex2";
        auto v1 = edge.getElementName(v1Name, MapToNamed);
        auto v2 = edge.getElementName(v2Name, MapToNamed);
        if (v1 != v1Name && v2 != v2Name) {
            res.setElementName(v1Name, v2);
            res.setElementName(v2Name, v1);
        }
        else if (v1 != v1Name && edge.countSubShapes(TopAbs_EDGE) == 1) {
...
TopoShape & TopoShape::makEBSplineFace(const std::vector<TopoShape> &input, FillingStyle style, const char *op)
{
    ...
    if (newEdges.size() != edges.size())
        ...
        for (auto &edge : newEdges)
            edge.resetElementMap(edges[i++]._ElementMap);
